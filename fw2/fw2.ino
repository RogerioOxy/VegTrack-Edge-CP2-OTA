...
void showState(State value) {
  setRgb(value == ALERTA, value == NORMAL, false);
  Serial.printf("Estado: %s; LED %s\n", value == ALERTA ? "ALERTA" : "NORMAL",
                value == ALERTA ? "vermelho" : "verde");
}

void selfTest() {
  Serial.println("DIAGNÓSTICO: vetores fixos, NÃO são leituras de campo.");
  Serial.println("Usa as mesmas funções de média, ordenação, mediana, histerese e LED.");
  const int vectors[][5] = {
    {18, 12, 15, 14, 20}, {16, 12, 18, 17, 20},
    {14, 16, 15, 15, 14}, {14, 14, 14, 14, 14},
    {15, 15, 15, 15, 15}, {10, 14, 12, 11, 13}
  };
  const int expectedMedian[] = {15, 17, 15, 14, 15, 12};
  const State expectedState[] = {NORMAL, ALERTA, ALERTA, NORMAL, NORMAL, NORMAL};
  State diagnosticState = NORMAL; // não altera o estado nem o vetor da produção
  for (int i = 0; i < 6; i++) {
    int sorted[5];
    int median = medianFive(vectors[i], sorted);
    diagnosticState = nextState(median, diagnosticState);
    Serial.printf("TESTE %d: mediana=%d estado=%s resultado=%s\n", i + 1, median,
                  diagnosticState == ALERTA ? "ALERTA" : "NORMAL",
                  median == expectedMedian[i] && diagnosticState == expectedState[i]
                    ? "PASS" : "FAIL");
    printVector("Original diagnóstico: ", vectors[i]);
    printVector("Ordenado diagnóstico: ", sorted);
    Serial.printf("Média diagnóstico: %.1f cm\n", meanFive(vectors[i]));
    showState(diagnosticState);
    delay(500); // antes da primeira sessão: permite observar as cores
  }
  const int at16[] = {16, 16, 16, 16, 16};
  int sorted[5];
  State boundary = nextState(medianFive(at16, sorted), NORMAL);
  Serial.printf("TESTE limite 16: %s\n", boundary == ALERTA ? "PASS" : "FAIL");
  showState(boundary);
  delay(500);
  showState(state); // restaura NORMAL antes das sessões pseudoaleatórias
  Serial.println("FIM DIAGNÓSTICO. Início das leituras de campo simuladas.");
}
...
void finishSession() {
  ...
  int sorted[5];
  int median = medianFive(readings, sorted);
  printVector("Ordem crescente: ", sorted);
  Serial.printf("Mediana (3º valor): %d cm\n", median);
  state = nextState(median, state);
  showState(state);
  completedSessions++;
